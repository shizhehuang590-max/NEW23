[index (35).html](https://github.com/user-attachments/files/27546527/index.35.html)
<!DOCTYPE html>
<html lang="zh-TW">
<head>
<meta charset="UTF-8">
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .cyan { color: #22d3ee; } .green { color: #4ade80; } .gold { color: #f0c040; } .red { color: #f87171; }
</style>
</head>
<body>

<h1>📊 3.5 GHz PA — Optimization Benchmark</h1>
<div class="sub">TSMC 28nm RF · 5 Algorithms · 10 Random Seeds (Pure Data Mode)</div>
...
function renderDash(methods, results) {
  ...
  methods.forEach(m => {
    ...
    data.forEach(row => {
      html += `<tr><td>${row.seed}</td><td class="cyan">${row.a.Gain.toFixed(2)}</td><td class="gold">${row.a.Pout_dBm.toFixed(2)}</td>
        <td class="green">${row.a.PAE.toFixed(1)}</td><td>${row.a.OP1dB.toFixed(2)}</td><td>${row.a.S11.toFixed(1)}</td>
        <td>${(row.fitness*100).toFixed(1)}%</td></tr>`;
    });
    html += `</table></div>`;
